...
std::vector<unsigned int> iList(MAXLINE, 0);
...
int main()
{
    ...
    {
        if (sscanf(line, "%u%u", &gp, &sizeOfWords) == 2)
        {
            iList.resize(gp + 1);
            regs[28] = gp;
            data.resize(sizeOfWords);
            printf("%d %d\n", gp, sizeOfWords);
        }
        else
        {
    ...
    while (counter <= iList.size())
    {
        ...
        else if (inst >> 26 == 2 || inst >> 26 == 3) // parse j
        {
